...
func AddFlags(ci *fs.ConfigInfo, flagSet *pflag.FlagSet) {
	...
	flags.IntVarP(flagSet, &ci.TrafficThreads, "traffic-threads", "", ci.TrafficThreads, "Number of traffic threads to run in parallel")
	...
	flags.IntVarP(flagSet, &ci.HighLevelRetries, "high-level-retries", "", ci.HighLevelRetries, "Number of high level retries to do")
	...
	flags.StringVarP(flagSet, &ci.TransferStartTime, "transfer-start-time", "", "", "Do all the checks before starting transfers (specify time UTC, using string like 2006-01-02 15:04:05)")
	...
	flags.StringVarP(flagSet, &ci.MultiThreadChunkSize, "multi-thread-chunksize", "", "", "Enable multi-thread download in chunk mode (to work on target fs without writerAt)")
	flags.IntVarP(flagSet, &ci.MultiThreadChunkAhead, "multi-thread-chunkahead", "", 5, "Set how many chunk can be download out-of-order (increase mem usage)")
